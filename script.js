...
const money_mius = document.getElementById('money-minus');
 ...
 //  Add transaction
   function addTransaction(e){
      ...
      if(text.value.trim() === '' || amount.value.trim() === ''){
         alert('Please add a value and amount')
      }else{
        const transaction = {
           id: generateID(),
           text: text.value,
     ...
     const amounts = transactions.map(transaction => 
      ...
      .toFixed(2);
       ...
       const income = amounts
            ...
            const expense = ( amounts.filter(item => item < 0).reduce((acc,
              item)=> (acc += item),0 )*
              -1
            ).toFixed(2);
